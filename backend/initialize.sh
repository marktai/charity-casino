...
if [ ! -f src/casino/keys.py ]; then
  echo "SECRET_KEY = '"$(cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 32 | head -n 1)"'" > src/casino/keys.py
  echo $(pwd)"/src/casino/keys.py generated"
fi
